...
pub const ModelListSnapshot = struct {
    ...
    fn catalogExplanation(self: ModelListSnapshot) ?[]const u8 {
        ...
        return switch (reason) {
            .no_credential => "Using the public model catalog; sign in with an OAuth provider or use an AI Gateway API key for team-private models.",
...
test "core status snapshot text and json stay stable" {
    const snapshot = StatusSnapshot{
        .model = "alpha",
        .auth_help = "Fx needs authentication. Configure an OAuth provider with FX_OAUTH_ISSUER_URL and FX_OAUTH_CLIENT_ID, run fx setup to use an API key, or set AI_GATEWAY_API_KEY.",
        .permission_mode = .ask,
    ...
    try std.testing.expectEqualStrings(
        "{\"kind\":\"status\",\"model\":\"alpha\",\"update_channel\":\"stable\",\"build_channel\":\"stable\",\"build_revision\":\"\",\"auth\":\"missing\",\"auth_refreshable\":false,\"auth_help\":\"Fx needs authentication. Configure an OAuth provider with FX_OAUTH_ISSUER_URL and FX_OAUTH_CLIENT_ID, run fx setup to use an API key, or set AI_GATEWAY_API_KEY.\",\"permission_mode\":\"ask\",\"sandbox\":\"none\",\"workspace\":\"/tmp/fx\",\"history_turns\":3,\"session_permission_grants\":1,\"agent_step_limit\":24}",
        json,
    );
}
...
test "model list explains public-only and rejected-credential catalogs" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .text = "[models] 1 available\n - alpha\n[models] Using the public model catalog; sign in with an OAuth provider or use an AI Gateway API key for team-private models.\n",
            .body = "1 available\n - alpha\nUsing the public model catalog; sign in with an OAuth provider or use an AI Gateway API key for team-private models.",
        },
        .{
            .snapshot = rejected,
