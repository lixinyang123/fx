...
pub const missing_credential_message = "Fx needs authentication. To sign in with OAuth, set FX_OAUTH_ISSUER_URL and FX_OAUTH_CLIENT_ID and run fx login; or use an API key via fx setup or AI_GATEWAY_API_KEY.";
pub const missing_interactive_credential_message = "Fx needs authentication. To sign in with OAuth, set FX_OAUTH_ISSUER_URL and FX_OAUTH_CLIENT_ID and run /login; or use an API key via /setup or AI_GATEWAY_API_KEY.";
