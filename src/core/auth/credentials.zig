...
pub const missing_credential_message = "Fx needs authentication. Configure an OAuth provider with FX_OAUTH_ISSUER_URL and FX_OAUTH_CLIENT_ID, then run fx login to sign in, fx setup to use an API key, or set AI_GATEWAY_API_KEY.";
pub const missing_interactive_credential_message = "Fx needs authentication. Configure an OAuth provider with FX_OAUTH_ISSUER_URL and FX_OAUTH_CLIENT_ID, then run /login to sign in, /setup to use an API key, or set AI_GATEWAY_API_KEY.";
